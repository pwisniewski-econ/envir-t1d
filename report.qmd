...
The Sanofi teams provided us with a dataset in .csv format. A summary of the data is presented in the table below:
...
data_sanofi = pd.read_csv("data/sanofi_data.csv")

print(build_table(
        data_sanofi[data_sanofi['description'].str.endswith(("arr24", "bv2022"))],
        include_pval=True,
        midrule_after=26,
...
Our primary objective was to isolate the target population from the 2023 PMSI Dataset. Given our main goal of identifying triggering or protective factors for type 1 diabetes, we aimed to focus on the incident population of 2023. To achieve this, we applied the following filters: we searched for individuals diagnosed with DP = E101 or DR = E10, indicating patients who were hospitalized with acidosis.

While this population does not perfectly represent the incident population, we estimate that it closely approximates it. Patients already diagnosed with type 1 diabetes are generally aware of the risk of acidosis and have the tools to monitor their glucose levels to prevent it. However, it is possible that some patients without acidosis may be hospitalized and subsequently diagnosed with type 1 diabetes. We hypothesize that this subgroup is relatively small compared to the population we filtered.
