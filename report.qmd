...
### Top 7 negative and positive features projected on PCA with all variables
...
def plot_top_loading_simple(df: pd.DataFrame,
    ...
    p = (
        ggplot(df_long, aes(x='Feature', y='Loading')) +
        geom_bar(stat='identity', fill='blue') +
        facet_wrap('~Component', ncol=1, scales='free_y') +
        coord_cartesian(ylim=(-1, 1)) +  # 👈 ici on fixe les limites
        theme_minimal() +
        theme(
            figure_size=(8, 6),
            axis_text_x=element_text(rotation=90, hjust=1),
        ) +
        labs(title=f'Top {top_n} positiv and negative variables for {col}',
             x='', y='')
    )
...
```{python}
#| fig-cap: Results of PCA for all variables - PC1
...
```{python}
#| fig-cap: Results of PCA for all variables - PC2
plot_top_loading_simple(
    df=load_p,
    col_names=['PC2'],
    top_n=7) 
...
```{python}
#| fig-cap: Results of PCA for all variables - PC3
plot_top_loading_simple(
    df=load_p,
    col_names=['PC3'],
    top_n=7) 
...
```{python}
#| fig-cap: Results of PCA for all variables - PC4
plot_top_loading_simple(
    df=load_p,
    col_names=['PC4'],
    top_n=7) 
...
```{python}
#| fig-cap: Results of PCA for all variables - PC5
plot_top_loading_simple(
    df=load_p,
    col_names=['PC5'],
    top_n=7) 
...
```{python}
#| fig-cap: Results of PCA for all variables - PC6
plot_top_loading_simple(
    df=load_p,
    col_names=['PC6'],
    top_n=7) 
